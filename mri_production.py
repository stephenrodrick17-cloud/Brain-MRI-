...
With 6-panel visualization and NO JavaScript errors
"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .left-panel h2 { color: #667eea; margin-bottom: 15px; font-size: 1.1em; }
        .drop-zone {
        ...
        .drop-zone:hover, .drop-zone.active { background: #667eea; color: white; }
        ...
        .tab-content { display: none; padding: 12px; overflow-y: auto; }
        .tab-content.active { display: block; }
        #canvas3d { width: 100%; height: 400px; }
        .visualization { width: 100%; margin-top: 10px; }
        .visualization img { width: 100%; border-radius: 6px; }
        .panel-description {
            font-size: 0.85em;
            line-height: 1.5;
            color: #555;
        ...
        .info { font-size: 0.85em; line-height: 1.5; color: #555; }
...
<body>
        ...
        <div class="content">
            ...
            <div class="left-panel">
                ...
                <div class="drop-zone" id="dropZone">
                    ...
                    <p style="font-size: 0.75em; margin-top: 3px;">or click</p>
                </div>
            ...
            <div class="right-panel">
                <div class="tabs">
                    <button class="tab-btn active" id="btnView3d">3D Viewer</button>
                    <button class="tab-btn" id="btnViz">6-Panel View</button>
                    <button class="tab-btn" id="btnMetrics">📊 Metrics</button>
                    <button class="tab-btn" id="btnInfo">ℹ️ Info</button>
                </div>

                <div class="tab-content active" id="tabView3d">
                    <canvas id="canvas3d"></canvas>
                </div>

                <div class="tab-content" id="tabViz">
                    <div class="panel-description">
                        <strong>6-Panel 3D Medical Image Reconstruction</strong><br>
                        <strong>Panel 1 (Top-Left):</strong> Estimated Depth Map - Shows tissue depth (blue=shallow, yellow=deep)<br>
                        ...
                        <strong>Panel 5 (Bottom-Center):</strong> Middle Axial Slice - Cross-sectional view at center<br>
                        <strong>Panel 6 (Bottom-Right):</strong> Intensity Histogram - Distribution of intensity values
                    </div>
    ...
    <script>
            ...
            fetch('/process', { method: 'POST', body: formData })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.viz_image) {
                            document.getElementById('vizContainer').innerHTML = '<img src="' + data.viz_image + '" alt="6-Panel Visualization">';
                        }
        ...
        function displayMetrics(results) {
            const html = `
                <div class="metric"><div class="metric-label">Dimensions (H×W×D)</div><div class="metric-value">${results.volume_dimensions.join('×')}</div></div>
                ...
                <div class="metric"><div class="metric-label">Std Deviation</div><div class="metric-value">${results.std_intensity.toFixed(3)}</div></div>
                ...
                <div class="metric"><div class="metric-label">Volume Coverage</div><div class="metric-value">${results.volume_coverage.toFixed(1)}%</div></div>
        ...
        function displayInfo(results) {
            const html = `
                ...
                <p><strong>Processing Time:</strong> ${new Date(results.timestamp).toLocaleString()}</p>
            `;
        ...
        // Tab switching - NO ONCLICK, using event listeners
        const tabButtons = {
            'btnView3d': 'tabView3d',
            'btnViz': 'tabViz',
            'btnMetrics': 'tabMetrics',
            'btnInfo': 'tabInfo'
        };
...
def process():
    try:
        ...
        ax1.set_title('Panel 1: Estimated Depth Map', fontweight='bold', fontsize=11)
        ax1.axis('off')
        ...
        ax2.set_title('Panel 2: Axial Projection (Top View)', fontweight='bold', fontsize=11)
        ax2.axis('off')
        ...
        ax3.set_title('Panel 3: Sagittal Projection (Side View)', fontweight='bold', fontsize=11)
        ax3.axis('off')
        ...
        ax4.set_title('Panel 4: Coronal Projection (Front View)', fontweight='bold', fontsize=11)
        ax4.axis('off')
        plt.colorbar(im4, ax=ax4, label='Intensity')
        
        # Panel 5: Middle Axial Slice
        ax5 = fig.add_subplot(2, 3, 5)
        mid_slice = volume_norm[volume_norm.shape[0]//2, :, :]
        im5 = ax5.imshow(mid_slice, cmap='viridis')
        ax5.set_title('Panel 5: Middle Axial Slice', fontweight='bold', fontsize=11)
        ax5.axis('off')
        ...
        ax6.set_title('Panel 6: Intensity Histogram', fontweight='bold', fontsize=11)
...
if __name__ == '__main__':
    print("\n" + "="*70)
    print("🏥 3D MRI RECONSTRUCTION SYSTEM - PRODUCTION READY")
    print("="*70)
    print("\n✓ Server: http://localhost:5000")
    print("✓ Features: 3D Viewer + 6-Panel Visualization + No Errors")
    print("\n" + "="*70 + "\n")
