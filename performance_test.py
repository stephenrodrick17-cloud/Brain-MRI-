...
def create_test_image():
    """Create a synthetic test medical image (3-channel, 512x512)"""
    np.random.seed(42)
    
    # Create synthetic medical image
    img = np.random.normal(100, 20, (512, 512, 3)).astype(np.uint8)
...
def test_performance():
    ...
    start = time.time()
    viz_path = pipeline.reconstructor.visualize_3d(
        recon_result,
        save_path=output_dir / 'visualization_perf_test.png',
    ...
    html_path = pipeline.reconstructor.create_interactive_html(
        ...
        findings='Performance test case - synthetic medical image',
    ...
    results = {
        ...
        'average_time_per_step': total_time / len(timing),
        'optimizations': [
